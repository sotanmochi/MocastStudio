...
{
    public sealed class MotionBuilderDataBuffer : IBodyTrackingDataSource, IFingerTrackingDataSource
    {
        private readonly BodyTrackingFrame[] _bodyTrackingDataBuffer;
        private readonly FingerTrackingFrame[] _fingerTrackingDataBuffer;
        ...
        public MotionBuilderDataBuffer(int id, int streamingDataId, int bufferSize = 2)
        {
            ...
            _bodyTrackingDataBuffer = new BodyTrackingFrame[bufferSize];
            _fingerTrackingDataBuffer = new FingerTrackingFrame[bufferSize];

            for (var i = 0; i < bufferSize; i++)
            {
                _bodyTrackingDataBuffer[i] = new BodyTrackingFrame();
                _fingerTrackingDataBuffer[i] = new FingerTrackingFrame();
            }
        }

        /// <summary>
        /// Try to get a reference of the data at the head in the buffer.
        ...
        public bool TryPeek(out BodyTrackingFrame bodyTrackingFrame)
        {
            ...
            bodyTrackingFrame = _bodyTrackingDataBuffer[bufferIndex];
            return true;
        }
        ...
        {
            var bufferIndex = _bufferHead & _bufferMask;
            fingerTrackingFrame = _fingerTrackingDataBuffer[bufferIndex];
            return true;
        }

        public void Enqueue(MotionBuilderStreamingReceiver.SkeletonData skeletonData)
        {
            if (skeletonData.ObjectId != StreamingDataId) return;

            var enqueueIndex = _bufferTail & _bufferMask;

            _bodyTrackingDataBuffer[enqueueIndex].RootPosition = skeletonData.RootBonePosition;
            _bodyTrackingDataBuffer[enqueueIndex].RootRotation = skeletonData.RootBoneRotation;
            _bodyTrackingDataBuffer[enqueueIndex].Scale = skeletonData.Scale;
            ...
            for (var i = 0; i < boneCount; i++)
            {
                ...
                if (bodyBoneId >= 0)
                {
                    _bodyTrackingDataBuffer[enqueueIndex].BoneRotations[bodyBoneId] = skeletonData.BoneRotations[i];
                }

                if (fingerBoneId >= 0)
                {
                    _fingerTrackingDataBuffer[enqueueIndex].BoneRotations[fingerBoneId] = skeletonData.BoneRotations[i];
                }
            }
